...
export function Header() {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center">
      {/* Premium Marquee Announcement Bar - Dark Mode Version */}
      <div className="w-full bg-slate-900 py-1.5 overflow-hidden whitespace-nowrap border-b border-slate-800">
        <div className="inline-block animate-marquee">
          {ANNOUNCEMENTS.map((text, index) => (
            <span key={index} className="inline-flex items-center px-12 text-[9px] font-bold uppercase tracking-[0.25em] text-white/90">
              <Sparkles className="w-2.5 h-2.5 mr-4 text-primary" />
              {text}
            </span>
          ))}
          {/* Duplicate for seamless loop */}
          {ANNOUNCEMENTS.map((text, index) => (
            <span key={`dup-${index}`} className="inline-flex items-center px-12 text-[9px] font-bold uppercase tracking-[0.25em] text-white/90">
              <Sparkles className="w-2.5 h-2.5 mr-4 text-primary" />
              {text}
            </span>
          ))}
